...
* Descrição Primária.                                                        *
* Descrições mais elaborada sobre o arquivo.                                 *
* \author brAthena                                                           *
* \date ?                                                                    *
* \todo ?                                                                    *
*****************************************************************************/ 
...
void pincode_handle (int fd, struct char_session_data* sd) {
	...
	if(strlen(sd->pincode) == 4) {
		if( *pincode->changetime && time(NULL) > (sd->pincode_change+*pincode->changetime) ){ // Usuário não mudou o seu código PIN por muito tempo
			pincode->sendstate( fd, sd, PINCODE_EXPIRED );
		} else { // Pedir usuário e seu código PIN
			pincode->sendstate( fd, sd, PINCODE_ASK );
		}
	} else // Não definiu-se ainda nenhum código PIN
